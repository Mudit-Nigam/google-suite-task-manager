...
db_client: MongoClient  # type: ignore
mydb: Database  # type: ignore
tasks: Collection  # type: ignore
db_name: str


# Define a function to get the database and tasks collection
def get_database():  # type: ignore
    global db_client, mydb, tasks, db_name
...
def add_task_db(mydb, task_id, task_name, task_details, due, user_id) -> None:  # type: ignore
...
def delete_task_db(mydb, task_id, user_id):  # type: ignore
...
def update_task_db(mydb, tasks, user_id):  # type: ignore
    ...
    delete_result = mydb.tasks.delete_many({"user": user_id, "_id": {"$nin": list(task_ids)}})
...
def fetch_task_db(mydb, user_id):  # type: ignore
